...
const updateUserOrderDrink = async (req, res, next) => {
    ...
    const productDrinkId = await params.productDrinkId;
        ...
        if (validateObjectId(restaurantId)) {
            ...
            if (validateObjectId(userId)) {
                ...
                if (validateObjectId(cartUserId)) {
                    ...
                    if (validateObjectId(productDrinkId)) {
                                ...
                                if (userExists) {
                                    ...
                                    if (cartUserExists) {
                                        ...
                                        if (order) {
                                            ...
                                            const index = order.myCart.drinks.findIndex(el => el._id == productDrinkId);

                                            if (order.myCart.drinks.includes(order.myCart.drinks[index])) {

                                                if (isServed === false || isServed === true) {

                                                    order.myCart.drinks[index].isServed = isServed
                                                    order.myCart.drinks[index].updatedAt = new Date();  
                                                    ...
                                                    try {
                                                        ...
                                                        return res.status(200).json({
                                                            ...
                                                            order: order.myCart.drinks[index]
                                                        })
                                                    } catch (error) {
                                                    ...
                                                    order.myCart.drinks[index].isPaid = isPaid
                                                    order.myCart.drinks[index].updatedAt = new Date();  
                        ...
                        res.status(404).json({message: "The parameter productDrinkId = " + productDrinkId + " in the URL does not fulfill the correct form of an objectId."});
                    }
...
export default updateUserOrderDrink;